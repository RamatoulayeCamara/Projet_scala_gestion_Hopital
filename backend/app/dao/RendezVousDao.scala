...
class RendezVousDao @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  // Dans le DAO RendezVousDao
  def getAllRendezVous(): Future[Seq[RendezVous]] = {
    db.run(rendezvous.result) // Récupérer tous les rendez-vous 
  }

  def getRendezVousById(id: Long): Future[Option[RendezVous]] = {
    db.run(rendezvous.filter(_.id === id).result.headOption) // Récupérer un rendez-vous par son ID
  }
